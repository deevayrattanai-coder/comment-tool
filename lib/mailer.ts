...
/**
 * Generic transactional email with a prominent CTA button.
 *
 * @param name     Recipient's display name.
 * @param link     URL the button points to.
 * @param heading  Button label and email heading (default: "Verify my email").
 * @param body     Intro paragraph shown above the button (default: verification copy).
 * @param expiry   Expiry note shown at the bottom (default: "24 hours").
 */
export function buildVerificationEmail(
  name: string,
  link: string,
  heading = 'Verify my email',
  body = `Hi ${name?.trim() || 'there'}, thanks for signing up for Comment tools. Please confirm your email address to activate your account.`,
  expiry = '24 hours',
) {
  ...
  return `
    <div style="font-family: -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; max-width:520px; margin:0 auto; padding:32px 24px; color:#0f172a;">
      ...
      <p style="margin:0; font-size:12px; color:#94a3b8;">
        This link expires in ${expiry}. If you did not request this, you can safely ignore this email.
      </p>
    </div>
  `;
}
