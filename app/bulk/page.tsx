...
export default function BulkPage() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (parsed.length === 0) {
        setError(
          "No valid rows found. Make sure your file has username and message columns.",
        );
        return;
      }
      if (parsed.length > 200) {
        setError(
          `Only up to 200 rows are supported per upload (your file has ${parsed.length}).`,
        );
        return;
      }
      setRows(parsed);
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (renderRef.current) {
          ...
          const safeName = (r.username || `row-${i + 1}`).replace(
            /[^a-z0-9_-]/gi,
            "_",
          );
      ...
      a.href = url;
      a.download = `Comment tools-${platform}-bulk.zip`;
      a.click();
  ...
  if (loading) {
    return (
      <SiteLayout>
        <div className="max-w-[1000px] mx-auto px-6 py-20 text-center text-muted-foreground">
          Loading…
        </div>
      </SiteLayout>
    );
      ...
      <section className="max-w-[1000px] mx-auto px-6 pt-12 pb-16">
        <div className="mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-[640px]">
            Upload a CSV or Excel file with your comments. We render every row
            as a pixel-perfect screenshot and download them all as a single ZIP
            archive.
          </p>
        </div>
          ...
          <div className="mb-6 rounded-xl border border-primary/30 bg-primary/5 p-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-foreground">
                Bulk is a Pro feature
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                You can preview and parse files on the Free plan, but generating
                the ZIP requires Pro or Business.
              </p>
            </div>
            <Link
              href="/pricing"
        ...
        <label className="block rounded-2xl border-2 border-dashed border-border bg-card p-8 cursor-pointer hover:border-primary/50 transition-colors text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Required columns: <code className="font-mono">username</code>,{" "}
            <code className="font-mono">message</code>. Optional:{" "}
            <code className="font-mono">likes</code>,{" "}
            <code className="font-mono">time</code>,{" "}
            <code className="font-mono">isVerified</code>.
          </p>
        </label>
            ...
            <div className="max-h-[280px] overflow-y-auto divide-y divide-border">
              {rows.slice(0, 50).map((r, i) => (
                ...
                >
                  ...
                  <span className="font-semibold text-foreground w-36 truncate">
                    {r.username}
                  </span>
